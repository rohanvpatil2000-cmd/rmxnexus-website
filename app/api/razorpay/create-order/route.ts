...
export async function POST(request: Request) {
  try {
    ...
    console.log("Creating Razorpay order:", { size, quantity, unitPrice, subtotal, discount, total, amountInPaise });
    ...
    const razorpayOrder = await razorpay.orders.create({
      ...
      currency: "INR",
      receipt: `rmx_${Date.now()}`,
      notes: {
        product: "Personalized Lithophane Lamp",
        size,
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Unable to create Razorpay order." },
      { status: 500 }
    );
  }
}
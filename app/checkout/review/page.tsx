...
function calculateOrder(
  ...
  const lithophane =
    normalizeLithophane(lithophaneValue);
...
export default function ReviewPage() {
  ...
  const [photoDetails, setPhotoDetails] =
    ...
    try {
      ...
      if (savedConfig) {
        try {
          ...
          if (
            parsed.photoStoragePath
          ) {
            setPhotoDetails({
              storagePath:
                parsed.photoStoragePath,
              originalName:
                parsed.photoStorageName ||
                parsed.photoName ||
                "",
              mimeType:
                parsed.photoStorageMimeType ||
                parsed.photoType ||
                "",
              fileSize:
                Number(
                  parsed.photoStorageSize ||
                    parsed.photoSize ||
                    0
                ),
            });
  ...
  const handlePlaceOrder =
    async () => {
      ...
      try {
        ...
        if (!currentPhotoPath) {
          throw new Error(
            "Your uploaded photo could not be found. Please go back and upload your photo again."
          );
        }
              ...
              body: JSON.stringify({
                ...
                photo: {
                  storagePath:
                    currentPhotoPath,
                  originalName:
                    photoDetails?.originalName ||
                    "",
                  mimeType:
                    photoDetails?.mimeType ||
                    "",
                  fileSize:
                    photoDetails?.fileSize ||
                    0,
                },
              }),
            }

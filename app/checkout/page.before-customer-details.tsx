...
const sizes = {
  mini: { name: "Mini", dimensions: "12 × 12 cm", price: 799 },
  standard: { name: "Standard", dimensions: "15 × 15 cm", price: 999 },
  large: { name: "Large", dimensions: "20 × 20 cm", price: 1399 },
  xl: { name: "XL", dimensions: "25 × 25 cm", price: 1899 },
} as const;
...
const frameColors = {
  black: { name: "Matte Black" },
  grey: { name: "Graphite Grey" },
} as const;
...
export default function CheckoutPage() {
  ...
  const sizeId = (params.get("size") || "standard") as SizeId;
  const lithophaneId = (params.get("lithophane") || "natural-white") as LithophaneId;
  const frameId = (params.get("frame") || "black") as FrameId;

  const size = sizes[sizeId] ?? sizes.standard;
  const lithophane = lithophaneColors[lithophaneId] ?? lithophaneColors["natural-white"];
  const frame = frameColors[frameId] ?? frameColors.black;

  const initialQuantity = Math.max(
    1,
    Math.min(10, Number(params.get("quantity") || 1) || 1)
  );
  ...
  const selectionFolder = `${sizeId}-${frameId}-${lithophaneId}`;

  const heroImage = `/images/lithophane/${selectionFolder}/01-hero.jpg`;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-10">
          ...
          <p className="mt-3 max-w-2xl text-gray-400">
            Upload your photo, confirm your configuration, and continue with
            your order details.
          </p>
        </div>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 md:p-7">
            <div className="mb-6">
              ...
              <p className="mt-2 text-sm text-gray-500">
                Use a clear, well-lit photo. JPG, PNG, or WEBP up to 10 MB.
              </p>
            </div>
            ...
            {!previewUrl ? (
              <label className="group flex min-h-[360px] cursor-pointer flex-col items-center justify-center rounded-3xl border border-dashed border-cyan-400/30 bg-cyan-400/[0.03] px-6 text-center transition hover:border-cyan-400/60 hover:bg-cyan-400/[0.06]">
                ...
                <p className="text-lg font-bold">Choose your photo</p>
                <p className="mt-2 text-sm text-gray-500">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                ...
                <p className="text-sm font-bold">Secure order</p>
                <p className="mt-1 text-xs text-gray-500">Your details stay private.</p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                <Check className="mb-2 text-cyan-400" size={20} />
                <p className="text-sm font-bold">Made to order</p>
                <p className="mt-1 text-xs text-gray-500">Printed for you.</p>
              </div>
            </div>
          </section>

          <aside className="h-fit rounded-3xl border border-white/10 bg-white/[0.03] p-5 md:p-7">
            <h2 className="text-xl font-bold">2. Confirm your order</h2>
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-black">
              <div className="aspect-square">
                ...
                <img
                  ...
                  alt={previewUrl ? "Your uploaded photo" : "Selected lithophane configuration"}
                  className="h-full w-full object-contain"
                />
              </div>
            </div>
            ...
            <div className="mt-6 rounded-2xl border border-cyan-400/15 bg-cyan-400/[0.04] p-4">
              ...
              <p className="mt-3 font-bold">{selectionText}</p>
